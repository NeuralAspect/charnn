...
learning_rate = 1e-1
embedding_size = vocab_size // 2

# model parameters
Wxh = (
    torch.randn(hidden_size, embedding_size) * 0.01
).requires_grad_()  # input to hidden
Whh = (
...
optimizer = torch.optim.Adagrad(
    [Wxh, Whh, Why, bh, by] + list(embedding.parameters()), lr=learning_rate
)
...
def lossFun_torch(inputs, targets, hprev):
    ...
    for t in range(len(inputs)):
        xs[t] = embedding(torch.tensor(inputs[t], dtype=torch.long)).unsqueeze(1)
...
def sample_torch(h, seed_ix, n):
    ...
    """
    x = embedding(torch.tensor(seed_ix, dtype=torch.long)).unsqueeze(1)
    ixes = []
    for t in range(n):
        ...
        x = embedding(torch.tensor(ix, dtype=torch.long)).unsqueeze(1)
        ixes.append(ix)
    return ixes
